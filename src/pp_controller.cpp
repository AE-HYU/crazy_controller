...
PP_Controller_Node::PP_Controller_Node()
: rclcpp::Node(
    "pp_controller_manager",
    rclcpp::NodeOptions()
      .allow_undeclared_parameters(true)
      .automatically_declare_parameters_from_overrides(true))
{
    if (!initialize()) {
        RCLCPP_ERROR(this->get_logger(), "Failed to initialize PP controller");
        return;
    }

    RCLCPP_INFO(this->get_logger(), "PP Controller initialized successfully");
}

bool PP_Controller_Node::initialize() {
    // Get essential parameters
    LUT_path_ = this->get_parameter("lookup_table_path").as_string();
    mode_ = "PP";  // Fixed to PP mode
    ...
    RCLCPP_INFO(this->get_logger(), "Initializing PP controller");
    init_pp_controller();
    ...
    sub_car_state_ = this->create_subscription<Odometry>("/odom", 10,
                     std::bind(&PP_Controller_Node::car_state_cb, this, std::placeholders::_1));
    sub_car_state_frenet_ = this->create_subscription<Odometry>("/frenet/odom", 10,
                           std::bind(&PP_Controller_Node::car_state_frenet_cb, this, std::placeholders::_1));
    sub_imu_ = this->create_subscription<sensor_msgs::msg::Imu>("/sensors/imu/raw", 10,
              std::bind(&PP_Controller_Node::imu_cb, this, std::placeholders::_1));
    ...
    timer_ = this->create_wall_timer(
        ...
        std::bind(&PP_Controller_Node::control_loop, this));

    RCLCPP_INFO(this->get_logger(), "PP Controller ready");
    return true;
}

void PP_Controller_Node::init_pp_controller() {
    RCLCPP_INFO(this->get_logger(), "Initializing PP controller...");

    const std::string pp_params_path = this->get_parameter("pp_params_path").as_string();
    RCLCPP_INFO(this->get_logger(), "Loading PP parameters from: %s", pp_params_path.c_str());
    declare_pp_dynamic_parameters_from_yaml(pp_params_path);
    ...
    // Create PP controller with logging callbacks (using PP_Controller)
    auto info = [this](const std::string& s){ RCLCPP_INFO(this->get_logger(), "[PP] %s", s.c_str()); };
    auto warn = [this](const std::string& s){ RCLCPP_WARN(this->get_logger(), "[PP] %s", s.c_str()); };

    pp_controller_ = std::make_unique<crazy_controller::PP_Controller>(
...
void PP_Controller_Node::declare_pp_dynamic_parameters_from_yaml(const std::string& yaml_path) {
    YAML::Node root = YAML::LoadFile(yaml_path);
    if (!root["crazy_controller"] || !root["crazy_controller"]["ros__parameters"]) {
        throw std::runtime_error("Invalid pp_params YAML: missing crazy_controller.ros__parameters");
    }
...
std::pair<double,double> PP_Controller_Node::pp_cycle() {
    auto res = pp_controller_->main_loop(
...
void PP_Controller_Node::car_state_frenet_cb(const Odometry::SharedPtr msg) {
...
void PP_Controller_Node::imu_cb(const sensor_msgs::msg::Imu::SharedPtr msg) {
...
void PP_Controller_Node::on_parameter_event(const rcl_interfaces::msg::ParameterEvent & event) {
    if (event.node != "/pp_controller_manager") return;
    if (!pp_controller_) return;

    auto getp = [&](const char* name){ return this->get_parameter(name).as_double(); };

    // Update PP controller parameters
    pp_controller_->set_t_clip_min(getp("t_clip_min"));
    pp_controller_->set_t_clip_max(getp("t_clip_max"));
    pp_controller_->set_m_l1(getp("m_l1"));
    pp_controller_->set_q_l1(getp("q_l1"));
    pp_controller_->set_speed_lookahead(getp("speed_lookahead"));
    pp_controller_->set_lat_err_coeff(getp("lat_err_coeff"));
    pp_controller_->set_acc_scaler_for_steer(getp("acc_scaler_for_steer"));
    pp_controller_->set_dec_scaler_for_steer(getp("dec_scaler_for_steer"));
    pp_controller_->set_start_scale_speed(getp("start_scale_speed"));
...
void PP_Controller_Node::publish_stop_command() {
    AckermannDriveStamped ack;
...
void PP_Controller_Node::shutdown_handler() {
...
std::shared_ptr<crazy_controller::PP_Controller_Node> g_node = nullptr;
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<crazy_controller::PP_Controller_Node>();
    ...
    RCLCPP_INFO(node->get_logger(), "Starting PP Controller Node with graceful shutdown");
