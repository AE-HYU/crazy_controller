#include "crazy_controller/aug_controller.hpp"
...
AUG_Controller_Node::AUG_Controller_Node()
: rclcpp::Node(
    "aug_controller_manager",
    rclcpp::NodeOptions()
      .allow_undeclared_parameters(true)
      .automatically_declare_parameters_from_overrides(true))
{
    if (!initialize()) {
        RCLCPP_ERROR(this->get_logger(), "Failed to initialize AUG controller");
        return;
    }

    RCLCPP_INFO(this->get_logger(), "AUG Controller initialized successfully");
}

bool AUG_Controller_Node::initialize() {
    // Get essential parameters
    LUT_path_ = this->get_parameter("lookup_table_path").as_string();
    mode_ = "AUG";  // Fixed to AUG mode
    ...
    sub_local_waypoints_ = this->create_subscription<WpntArray>("/local_waypoints", 10,
                          std::bind(&AUG_Controller_Node::local_waypoint_cb, this, std::placeholders::_1));
    sub_car_state_ = this->create_subscription<Odometry>("/odom", 10,
                     std::bind(&AUG_Controller_Node::car_state_cb, this, std::placeholders::_1));
    sub_car_state_frenet_ = this->create_subscription<Odometry>("/frenet/odom", 10,
                           std::bind(&AUG_Controller_Node::car_state_frenet_cb, this, std::placeholders::_1));
    sub_imu_ = this->create_subscription<sensor_msgs::msg::Imu>("/sensors/imu/raw", 10,
              std::bind(&AUG_Controller_Node::imu_cb, this, std::placeholders::_1));
    ...
    timer_ = this->create_wall_timer(
        ...
        std::bind(&AUG_Controller_Node::control_loop, this));

    RCLCPP_INFO(this->get_logger(), "AUG Controller ready");
    return true;
}

void AUG_Controller_Node::init_aug_controller() {
    RCLCPP_INFO(this->get_logger(), "Initializing AUG controller...");

    const std::string aug_params_path = this->get_parameter("aug_params_path").as_string();
    RCLCPP_INFO(this->get_logger(), "Loading AUG parameters from: %s", aug_params_path.c_str());
    declare_aug_dynamic_parameters_from_yaml(aug_params_path);
    ...
    auto info = [this](const std::string& s){ RCLCPP_INFO(this->get_logger(), "[AUG] %s", s.c_str()); };
    auto warn = [this](const std::string& s){ RCLCPP_WARN(this->get_logger(), "[AUG] %s", s.c_str()); };

    aug_controller_ = std::make_unique<crazy_controller::AUG_Controller>(
        ...
        LUT_path_,
        this->get_parameter("Cf").as_double(),
        this->get_parameter("Cr").as_double(),
        this->get_parameter("L").as_double(),
        this->get_parameter("lf").as_double(),
        this->get_parameter("lr").as_double(),
        this->get_parameter("m").as_double(),
        this->get_parameter("kf").as_double(),
        this->get_parameter("kr").as_double(),
        info, warn);

    RCLCPP_INFO(this->get_logger(), "✓ AUG controller initialized successfully");
}

void AUG_Controller_Node::declare_aug_dynamic_parameters_from_yaml(const std::string& yaml_path) {
    YAML::Node root = YAML::LoadFile(yaml_path);
    if (!root["crazy_controller"] || !root["crazy_controller"]["ros__parameters"]) {
        throw std::runtime_error("Invalid aug_params YAML: missing crazy_controller.ros__parameters");
    }
    ...
    auto declare_string = [&](const std::string& name, const std::string& def) {
        ParameterDescriptor desc;
        desc.type = ParameterType::PARAMETER_STRING;
        (void)this->declare_parameter<std::string>(name, def, desc);
    };
    ...
    // Vehicle parameters
    declare_double("Cf", params["Cf"].as<double>(), fp(0.0, 200.0, 0.001));
    declare_double("Cr", params["Cr"].as<double>(), fp(0.0, 200.0, 0.001));
    declare_double("L", params["L"].as<double>(), fp(0.0, 0.5, 0.0001));
    declare_double("lf", params["lf"].as<double>(), fp(0.0, 0.5, 0.00001));
    declare_double("lr", params["lr"].as<double>(), fp(0.0, 0.5, 0.00001));
    declare_double("m", params["m"].as<double>(), fp(0.0, 10.0, 0.01));

    // Tire stiffness adjustment coefficients
    declare_double("kf", params["kf"].as<double>(), fp(0.0, 0.01, 0.00001));
    declare_double("kr", params["kr"].as<double>(), fp(0.0, 0.01, 0.00001));
}

void AUG_Controller_Node::wait_for_messages() {
    RCLCPP_INFO(this->get_logger(), "AUG Controller waiting for messages...");
    ...
    RCLCPP_INFO(this->get_logger(), "✓ All required messages received. AUG Controller ready to start!");
}

void AUG_Controller_Node::control_loop() {
    ...
    if (timing_log_counter_ >= 200) {  // 5 seconds at 40Hz
        RCLCPP_INFO(this->get_logger(), "[AUG] Control loop execution time: %.3f ms",
                    duration.count() / 1000.0);
        timing_log_counter_ = 0;
    }
}

std::pair<double,double> AUG_Controller_Node::aug_cycle() {
    auto res = aug_controller_->main_loop(
...
// Callback functions
void AUG_Controller_Node::local_waypoint_cb(const WpntArray::SharedPtr msg) {
...
void AUG_Controller_Node::car_state_cb(const Odometry::SharedPtr msg) {
    // Only update velocity from odom message (position updated from TF in control loop)
    speed_now_ = msg->twist.twist.linear.x;
}

bool AUG_Controller_Node::update_position_from_tf() {
    try {
...
void AUG_Controller_Node::car_state_frenet_cb(const Odometry::SharedPtr msg) {
...
void AUG_Controller_Node::imu_cb(const sensor_msgs::msg::Imu::SharedPtr msg) {
...
void AUG_Controller_Node::on_parameter_event(const rcl_interfaces::msg::ParameterEvent & event) {
    if (event.node != "/aug_controller_manager") return;
    if (!aug_controller_) return;

    auto getp = [&](const char* name){ return this->get_parameter(name).as_double(); };

    // Update AUG controller parameters
    aug_controller_->set_t_clip_min(getp("t_clip_min"));
    aug_controller_->set_t_clip_max(getp("t_clip_max"));
    aug_controller_->set_m_l1(getp("m_l1"));
    aug_controller_->set_q_l1(getp("q_l1"));
    aug_controller_->set_speed_lookahead(getp("speed_lookahead"));
    aug_controller_->set_lat_err_coeff(getp("lat_err_coeff"));
    aug_controller_->set_acc_scaler_for_steer(getp("acc_scaler_for_steer"));
    aug_controller_->set_dec_scaler_for_steer(getp("dec_scaler_for_steer"));
    aug_controller_->set_start_scale_speed(getp("start_scale_speed"));
    aug_controller_->set_end_scale_speed(getp("end_scale_speed"));
    aug_controller_->set_downscale_factor(getp("downscale_factor"));
    aug_controller_->set_speed_lookahead_for_steer(getp("speed_lookahead_for_steer"));
    aug_controller_->set_diff_threshold(getp("diff_threshold"));
    aug_controller_->set_deacc_gain(getp("deacc_gain"));
    aug_controller_->set_kf(getp("kf"));
    aug_controller_->set_kr(getp("kr"));
    aug_controller_->set_Cf(getp("Cf"));
    aug_controller_->set_Cr(getp("Cr"));

    RCLCPP_INFO(this->get_logger(), "Updated AUG parameters");
}

// Emergency stop functions
void AUG_Controller_Node::publish_stop_command() {
    AckermannDriveStamped ack;
...
void AUG_Controller_Node::shutdown_handler() {
...
std::shared_ptr<crazy_controller::AUG_Controller_Node> g_node = nullptr;
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<crazy_controller::AUG_Controller_Node>();
    ...
    RCLCPP_INFO(node->get_logger(), "Starting AUG Controller Node with graceful shutdown");
