...
class AUG_Controller; // Forward declaration

class AUG_Controller_Node : public rclcpp::Node {
public:
    AUG_Controller_Node();
    ~AUG_Controller_Node() = default;
...
private:
    // Core functionality
    void control_loop();
    std::pair<double,double> aug_cycle();

    // Initialization
    bool initialize();
    void init_aug_controller();
    void declare_aug_dynamic_parameters_from_yaml(const std::string& yaml_path);
    ...
    std::unique_ptr<crazy_controller::AUG_Controller> aug_controller_;
